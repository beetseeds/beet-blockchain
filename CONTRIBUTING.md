...
This repo includes the code for the beet full node, farmer, and timelord (in beet folder), which are all written in python.
It also includes a verifiable delay function implementation that it imports from the [chiavdf repo](https://github.com/beet-Network/chiavdf) (in c/c++), and a proof of space implementation that it imports from the [chiapos repo](https://github.com/beet-Network/chiapos). BLS signatures are imported from the [bls-signatures repo](https://github.com/beet-Network/bls-signatures) as blspy. There is an additional dependency on the [chiabip158 repo](https://github.com/beet-Network/chiabip158). For major platforms, binary and source wheels are shipped to PyPI from each dependent repo. Then beet-blockchain can pip install those from PyPI or they can be prepackaged as is done for the Windows installer. On unsupported platforms, pip will fall back to the source distributions, to be compiled locally.
...
Members of the beet organization may create feature branches from the `main` branch.

In the event an emergency fix is required for the release version of beet, members of the beet organization will create a feature branch from the current release branch `1.0.0`.
...
4. beet Network's nodes on the public testnet are running the latest version `x.y.z`.
...
. ./activate
pip install ".[dev]"
black beet tests && mypy beet tests && flake8 beet tests
...
7. Preferences > Settings > mypy > Targets: set to `./beet` and `./tests`
...
6. Install these linters https://github.com/beet-Network/beet-blockchain/tree/main/.github/linters

## Testnets and review environments

With the launch of `1.0.0` we will begin running an official `testnet`.  
Prior to the release of `1.1.0` there will be two running test nets. `testnet` and `transaction-beta-testnet`. The `transaction-beta-testnet` testnet will be a beta of the pending 1.1 release, which will enable transactions on the beet blockchain.
...
beet organization members have been granted CI access to deploy `beta testnets`.
If you are not a beet organization member, you can enquire about deploying a `beta testnet` in the public dev Discord channel.

## Submit changes

To propose changes, please make a pull request to the `main` branch. See Branching Strategy above.

To propose changes for the production releases of beet, please make a pull request to the latest release branch.

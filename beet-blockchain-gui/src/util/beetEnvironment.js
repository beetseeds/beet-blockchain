...
const getbeetVersion = () => {
  let version = null;
  const exePath = getExecutablePath('beet');
  // first see if we can get a beet exe in a standard location relative to where we are
  try {
    ...
    // that didn't work, let's try as if we're in the venv or beet is on the path
    try {
      version = child_process

...
const IncompleteCard = (props) => {
  ...
  function submit() {
    ...
    const beetper_input = ip_parsed.limit;
    ...
    const beetper_value = Number.parseInt(Number(beetper_input));
        ...
        <Grid item xs={12}>
          <Box display="flex">
            ...
            <Box flexGrow={1}>
              <Typography variant="subtitle1">
                <Trans>
                  Spending Limit (beet per interval):{' '}
                  {mojo_to_beet_string(limit)}
                </Trans>
              </Typography>
            </Box>
          </Box>
...
const SendCard = (props) => {
  ...
  function send() {
    ...
    const fee = beet_to_mojo(fee_input.value);

...
from beet.cmds.init_funcs import beet_full_version_str
from beet.protocols.protocol_message_types import ProtocolMessageTypes
from beet.protocols.shared_protocol import Capability, Handshake
...
from beet.util.ints import uint8, uint16

# Each message is prepended with LENGTH_BYTES bytes specifying the length
from beet.util.network import class_for_type, is_localhost

# Max size 2^(8*4) which is around 4GiB
LENGTH_BYTES: int = 4


class WSbeetConnection:
    """
    ...
    async def perform_handshake(self, network_id: str, protocol_version: str, server_port: int, local_type: NodeType):
        if self.is_outbound:
            ...
            if inbound_handshake.network_id != 'beet-' + network_id:

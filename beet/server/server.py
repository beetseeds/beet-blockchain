...
from beet.server.introducer_peers import IntroducerPeers
from beet.server.outbound_message import Message, NodeType
from beet.server.ssl_context import private_ssl_paths, public_ssl_paths
...
from beet.util.errors import Err, ProtocolError
from beet.util.ints import uint16
from beet.util.network import is_localhost, is_in_network
...
class beetServer:
    def __init__(
        ...
        beet_ca_crt_key: Tuple[Path, Path],
        name: str = None,
        introducer_peers: Optional[IntroducerPeers] = None,
    ):
        # Keeps track of all connections to and from this node.
        logging.basicConfig(level=logging.DEBUG)
        self.all_connections: Dict[bytes32, WSbeetConnection] = {}
        self.tasks: Set[asyncio.Task] = set()

        self.connection_by_type: Dict[NodeType, Dict[bytes32, WSbeetConnection]] = {
            NodeType.FULL_NODE: {},
            NodeType.WALLET: {},
            NodeType.HARVESTER: {},
            NodeType.FARMER: {},
        ...
        self.beet_ca_crt_path, self.beet_ca_key_path = beet_ca_crt_key
    ...
    async def garbage_collect_connections_task(self) -> None:
        ...
        while True:
            ...
            to_remove: List[WSbeetConnection] = []
    ...
    async def start_server(self, on_connect: Callable = None):
        ...
        if authenticate:
            ...
            ssl_context = ssl_context_for_server(
                self.beet_ca_crt_path, self.beet_ca_key_path, self.p2p_crt_path, self.p2p_key_path
            )
    ...
    async def incoming_connection(self, request):
        ...
        if peer_id == self.node_id:
            return ws
        connection: Optional[WSbeetConnection] = None
        try:
            connection = WSbeetConnection(
                self._local_type,
                ws,
                self._port,
                self.log,
    ...
    async def connection_added(self, connection: WSbeetConnection, on_connect=None):
    ...
    def connection_closed(self, connection: WSbeetConnection, ban_time: int):
    ...
    async def incoming_api_task(self) -> None:
        ...
        while True:
            ...
            async def api_call(full_message: Message, connection: WSbeetConnection, task_id):
    ...
    def get_outgoing_connections(self) -> List[WSbeetConnection]:
        result = []
        for _, connection in self.all_connections.items():
    ...
    def get_full_node_outgoing_connections(self) -> List[WSbeetConnection]:
        result = []
    ...
    def get_full_node_connections(self) -> List[WSbeetConnection]:
        return list(self.connection_by_type[NodeType.FULL_NODE].values())

    def get_connections(self, node_type: Optional[NodeType] = None) -> List[WSbeetConnection]:
        result = []
        for _, connection in self.all_connections.items():
    ...
    def is_trusted_peer(self, peer: WSbeetConnection, trusted_peers: Dict) -> bool:

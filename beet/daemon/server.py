...
from beet.cmds.init_funcs import beet_init
from beet.daemon.windows_signal import kill
from beet.server.server import ssl_context_for_root, ssl_context_for_server
from beet.ssl.create_ssl import get_mozilla_ca_crt
from beet.util.beet_logging import initialize_logging
from beet.util.config import load_config
from beet.util.json_util import dict_to_json_str
from beet.util.path import mkdir
from beet.util.service_groups import validate_service
from beet.util.setproctitle import setproctitle
from beet.util.ws_message import WsRpcMessage, create_payload, format_response
...
try:
    ...
    print("Error: Make sure to run . ./activate from the project folder before starting beet.")
    quit()
...
service_plotter = "beet plots create"
...
if getattr(sys, "frozen", False):
    name_map = {
        "beet": "beet",
        "beet_wallet": "start_wallet",
        "beet_full_node": "start_full_node",
        "beet_harvester": "start_harvester",
        "beet_farmer": "start_farmer",
        "beet_introducer": "start_introducer",
        "beet_timelord": "start_timelord",
        "beet_timelord_launcher": "timelord_launcher",
        "beet_full_node_simulator": "start_simulator",
    }
...
class WebSocketServer:
    ...
    async def exit(self) -> Dict[str, Any]:
        ...
        log.info("beet daemon exiting in 5 seconds")
...
def launch_plotter(root_path: Path, service_name: str, service_array: List[str], id: str):
    # we need to pass on the possibly altered beet_ROOT
    os.environ["beet_ROOT"] = str(root_path)
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    log.debug(f"Launching service with beet_ROOT: {os.environ['beet_ROOT']}")

...
from beet.pools.pool_puzzles import SINGLETON_LAUNCHER_HASH
from beet.protocols import wallet_protocol
from beet.protocols.full_node_protocol import RequestProofOfWeight, RespondProofOfWeight
from beet.protocols.protocol_message_types import ProtocolMessageTypes
from beet.protocols.wallet_protocol import (
...
from beet.server.node_discovery import WalletPeers
...
from beet.util.ints import uint32, uint128
from beet.util.keychain import Keychain
from beet.util.lru_cache import LRUCache
from beet.util.merkle_set import MerkleSet, confirm_included_already_hashed, confirm_not_included_already_hashed
...
from beet.wallet.settings.settings_objects import BackupInitialized
...
from beet.wallet.wallet_blockchain import ReceiveBlockResult
from beet.wallet.wallet_state_manager import WalletStateManager
...
class WalletNode:
    ...
    def get_key_for_fingerprint(self, fingerprint: Optional[int]) -> Optional[PrivateKey]:
        ...
        if len(private_keys) == 0:
            self.log.warning("No keys present. Create keys with the UI, or with the 'beet keys' program.")
            return None
    ...
    async def on_connect(self, peer: WSbeetConnection):
        if self.wallet_state_manager is None or self.backup_initialized is False:
            return None
    ...
    async def complete_blocks(self, header_blocks: List[HeaderBlock], peer: WSbeetConnection):
    ...
    async def new_peak_wallet(self, peak: wallet_protocol.NewPeakWallet, peer: WSbeetConnection):
        if self.wallet_state_manager is None:
            return
    ...
    async def _sync(self) -> None:
        ...
        peers: List[WSbeetConnection] = self.server.get_full_node_connections()
        if len(peers) == 0:
    ...
    async def get_additions(
        self, peer: WSbeetConnection, block_i, additions: Optional[List[bytes32]], get_all_additions: bool = False
    ) -> Optional[List[Coin]]:
    ...
    async def get_removals(
        self, peer: WSbeetConnection, block_i, additions, removals, request_all_removals=False
    ) -> Optional[List[Coin]]:
        assert self.wallet_state_manager is not None

...
from beet.consensus.block_creation import create_unfinished_block
from beet.consensus.block_record import BlockRecord
from beet.consensus.pot_iterations import calculate_ip_iters, calculate_iterations_quality, calculate_sp_iters
from beet.full_node.bundle_tools import best_solution_generator_from_template, simple_solution_generator
from beet.full_node.full_node import FullNode
from beet.full_node.mempool_check_conditions import get_puzzle_and_solution_for_coin
from beet.full_node.signage_point import SignagePoint
from beet.protocols import farmer_protocol, full_node_protocol, introducer_protocol, timelord_protocol, wallet_protocol
from beet.protocols.full_node_protocol import RejectBlock, RejectBlocks
from beet.protocols.protocol_message_types import ProtocolMessageTypes
from beet.protocols.wallet_protocol import PuzzleSolutionResponse, RejectHeaderBlocks, RejectHeaderRequest
from beet.server.outbound_message import Message, make_msg
...
from beet.types.mempool_item import MempoolItem
from beet.types.peer_info import PeerInfo
from beet.types.unfinished_block import UnfinishedBlock
from beet.util.api_decorators import api_request, peer_required, bytes_required, execute_task
from beet.util.generator_tools import get_block_header
...
class FullNodeAPI:
    ...
    @peer_required
    @api_request
    async def request_peers(self, _request: full_node_protocol.RequestPeers, peer: ws.WSbeetConnection):
    ...
    async def respond_peers(
        self, request: full_node_protocol.RespondPeers, peer: ws.WSbeetConnection
    ) -> Optional[Message]:
    ...
    async def respond_peers_introducer(
        self, request: introducer_protocol.RespondPeersIntroducer, peer: ws.WSbeetConnection
    ) -> Optional[Message]:
    ...
    @peer_required
    @api_request
    async def new_peak(self, request: full_node_protocol.NewPeak, peer: ws.WSbeetConnection) -> Optional[Message]:
        """
    ...
    async def new_transaction(
        self, transaction: full_node_protocol.NewTransaction, peer: ws.WSbeetConnection
    ) -> Optional[Message]:
        """
    ...
    async def new_signage_point_or_end_of_sub_slot(
        self, new_sp: full_node_protocol.NewSignagePointOrEndOfSubSlot, peer: ws.WSbeetConnection
    ) -> Optional[Message]:
        # Ignore if syncing
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    async def respond_signage_point(
        self, request: full_node_protocol.RespondSignagePoint, peer: ws.WSbeetConnection
    ) -> Optional[Message]:
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    async def declare_proof_of_space(
        self, request: farmer_protocol.DeclareProofOfSpace, peer: ws.WSbeetConnection
    ) -> Optional[Message]:
        """
    ...
    async def signed_values(
        self, farmer_request: farmer_protocol.SignedValues, peer: ws.WSbeetConnection
    ) -> Optional[Message]:
        """
    ...
    async def new_infusion_point_vdf(
        self, request: timelord_protocol.NewInfusionPointVDF, peer: ws.WSbeetConnection
    ) -> Optional[Message]:
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    async def new_signage_point_vdf(
        self, request: timelord_protocol.NewSignagePointVDF, peer: ws.WSbeetConnection
    ) -> None:
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    async def new_end_of_sub_slot_vdf(
        self, request: timelord_protocol.NewEndOfSubSlotVDF, peer: ws.WSbeetConnection
    ) -> Optional[Message]:
        if self.full_node.sync_store.get_sync_mode():
            return None
        if (

...
ROOT = pathlib.Path(importlib.import_module("beet").__file__).absolute().parent.parent


def solve_name_collision_problem(analysis):
    """
    There is a collision between the `beet` file name (which is the executable)
    and the `beet` directory, which contains non-code resources like `english.txt`.
    We move all the resources in the zipped area so there is no
    need to create the `beet` directory, since the names collide.
...
version_data = copy_metadata(get_distribution("beet-blockchain"))[0]
...
# TODO: collapse all these entry points into one `beet_exec` entrypoint that accepts the server as a parameter

entry_points = ["beet.cmds.beet"] + [f"beet.server.start_{s}" for s in SERVERS]
...
if THIS_IS_WINDOWS:
    entry_points.extend(["aiohttp", "beet.util.bip39"])

if THIS_IS_WINDOWS:
    beet_mod = importlib.import_module("beet")
    dll_paths = ROOT / "*.dll"
...
datas.append((f"{ROOT}/beet/util/english.txt", "beet/util"))
datas.append((f"{ROOT}/beet/util/initial-config.yaml", "beet/util"))
datas.append((f"{ROOT}/beet/wallet/puzzles/*.hex", "beet/wallet/puzzles"))
datas.append((f"{ROOT}/beet/ssl/*", "beet/ssl"))
...
add_binary("beet", f"{ROOT}/beet/cmds/beet.py", COLLECT_ARGS)
add_binary("daemon", f"{ROOT}/beet/daemon/server.py", COLLECT_ARGS)

for server in SERVERS:
    add_binary(f"start_{server}", f"{ROOT}/beet/server/start_{server}.py", COLLECT_ARGS)

...
from beet.cmds.units import units
...
from beet.util.network import is_localhost
...
async def summary(
    ...
    if amounts is not None:
        print(f"Total beet farmed: {amounts['farmed_amount'] / units['beet']}")
        print(f"User transaction fees: {amounts['fee_amount'] / units['beet']}")
        print(f"Block rewards: {(amounts['farmer_reward_amount'] + amounts['pool_reward_amount']) / units['beet']}")
    ...
    if amounts is None:
        if wallet_not_running:
            print("For details on farmed rewards and fees you should run 'beet start wallet' and 'beet wallet show'")
        elif wallet_not_ready:
            print("For details on farmed rewards and fees you should run 'beet wallet show'")
    else:
        print("Note: log into your key using 'beet wallet show' to see rewards for each key")
